<?php
    class DB {
        ...
        function search_bitacora($id=null, $limite=9, $n=0, $order=" fecha DESC") {
            $query = "SELECT
                    b.id,
                    (SELECT nombre FROM usuarios WHERE usuarios.usuario = b.usuario) as nombre,
                    b.tabla,
                    b.accion,
                    b.fecha,
                    b.detalles
                    FROM bitacora b
                    INNER JOIN usuarios u ON b.usuario = u.usuario";
            if ($id != null){
        ...
        function COUNT_user($user) {
            $query = $this->conn->prepare("SELECT COUNT(*) as 'total' FROM bitacora WHERE usuario=:usuario");
            $query->bindParam(":usuario",$user, PDO::PARAM_INT);
            $query->execute();
            return $query->fetch()['total'];
