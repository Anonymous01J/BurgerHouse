...
use Facebook\WebDriver\Remote\RemoteWebElement;
    ...
    class ComunSelenium{
        ...
        public function login($clave = "Hola123", $usuario = "garnicaluis391@gmail.com"){
            ...
            try {
                ...
                $this->fillForms([
                    ...
                    ['selector' => '#login-password', 'value' => $clave],
                ]);
                $btnSbmit= $this->waitElement("button[type='submit']");
                $this->driver->executeScript("arguments[0].disabled = false;", [$btnSbmit]);
                $btnSbmit->click();
                $this->waitUrl(url("home"), 5);
            } catch (\Exception $th) {
                $this->print("No se pudo iniciar sesi n automáticamente, por favor inicie sesi n manualmente.",3);
            }
        ...
        public function fillForm ($selector, $value, $timeout = 3, $interval = 500, $mensaje = 'Elemento no encontrado'){
            try {
         ...
         */
        public function scrollTo($selector, $timeout = 3, $interval = 500) {
