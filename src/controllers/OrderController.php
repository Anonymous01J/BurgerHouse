...
function add_many()
{
    base_add_many('orders');
}
function delete_many()
{
    base_delete_many('orders');
}
function updateMany()
{
    base_update_many('orders');
}
function guardar_imagen_mult($index)
{
    base_guardar_imagen_mult('orders', $index);
}
function guardar_imagen_single()
{
    base_guardar_imagen_single('orders');
}
function add()
{
    try {
        ...
        if (in_array($_POST['tipo'], ["delivery", "llevar", "local"])) {
            if (isset($_POST['lista_detalle_preparado'])) {
                ...
                $result = VerifyPrepared($detalles_receta);
            ...
            if (isset($_POST['lista_detalle_procesado'])) {
                ...
                $result = VeryfyProcess($detalles_productos);
            ...
            if ($ok) {
                $orden->__construct(tipo: $_POST['tipo'], id_cliente: $_POST['id_cliente'] ?? null, status: $_POST['status'], nro_orden: $_POST['nro_orden']);
                $last_id = $orden->agregar();
                if (isset($_POST['lista_detalle_preparado'])) {
                    for ($i = 0; $i < count($_POST['lista_detalle_preparado']); $i++) {
                    ...
                    descountPrepared($detalles_receta);
                    verifyStockPrepared($detalles_receta);
                }
                if (isset($_POST['lista_detalle_procesado'])) {
                    ...
                    descountProcess($detalles_productos);
                    verifyStockProcess($detalles_productos);
                }
...
function add_process_and_prepared()
{
    try {
        $l = $_POST;
        $result_detalle_preparado = true;
...
function getDropboxAccessToken($clientId, $clientSecret, $refreshToken)
{
...
function sendInvoice()
{
    ...
    try {
        ...
        $accessToken = getDropboxAccessToken($clientId, $clientSecret, $refreshToken);
...
function VerifyPrepared($detalle_receta)
{
...
function descountPrepared($detalle_receta)
{
    date_default_timezone_set('America/Caracas');
    $entradas_materia_prima = new Detalle_entrada_materia_prima();
    $entradas_materia_prima->__construct(active: 1);
...
function VeryfyProcess($detalle_producto)
{
    date_default_timezone_set('America/Caracas');
...
function descountProcess($detalle_producto)
{
    date_default_timezone_set('America/Caracas');
    $entradas_process = new Entrada_producto_procesado();
    $entradas_process->__construct(active: 1);
...
function verifyStockProcess($detalle_producto)
{
    $productos = new ProductoProcesado();
...
function verifyStockPrepared($detalles_receta)
{
