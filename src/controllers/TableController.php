...
class TableController extends Controller_base
{
    ...
    {
        $imagen = $_FILES['lista'];
        move_uploaded_file($imagen['tmp_name'][$index]['imagen'], '../src/media/mesas/' . $imagen['name'][$index]['imagen']);
    }
    public function guardar_imagen_single()
    {
        $imagen = $_FILES['imagen'];
        move_uploaded_file($imagen['tmp_name'], '../src/media/mesas/' . $imagen['name']);
    }
}
