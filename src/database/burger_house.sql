...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 27-05-2025 a las 01:43:51
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `CerrarCaja` (IN `id_caja` INT)   BEGIN
    DECLARE total_ventas INT DEFAULT 0;
    DECLARE total_pagado_USD FLOAT DEFAULT 0;
    DECLARE total_pagado_BS FLOAT DEFAULT 0;

    SELECT COUNT(id) INTO total_ventas FROM ventas WHERE id_caja = id_caja;

    SELECT COALESCE(SUM(p.monto), 0) INTO total_pagado_USD 
    FROM pagos p 
    INNER JOIN ventas v ON p.id_venta = v.id
    WHERE v.id_caja = id_caja AND p.id_metodo_pago = 9;

    SELECT COALESCE(SUM(p.monto), 0) INTO total_pagado_BS
    FROM pagos p 
    INNER JOIN ventas v ON p.id_venta = v.id
    WHERE v.id_caja = id_caja AND p.id_metodo_pago = 4;

    UPDATE caja 
    SET monto_final_usd = total_pagado_USD + monto_inicial_$, 
        monto_final_bs = total_pagado_BS + monto_inicial_bs, 
        fecha_cierre = CURRENT_TIMESTAMP, 
        estado = 0, 
        total_ventas = total_ventas
    WHERE id = id_caja AND estado = 0;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `DescontarIngredientes` (IN `p_id_producto` INT, IN `p_cantidad` INT)   BEGIN
    DECLARE v_id_materia_prima INT;
    DECLARE v_cantidad_necesaria FLOAT;
    DECLARE done INT DEFAULT FALSE;

    -- Cursor para recorrer los ingredientes de la receta asociada al producto
    DECLARE cur_ingredientes CURSOR FOR 
        SELECT dr.id_materia_prima, dr.cantidad 
        FROM detalles_receta dr
        INNER JOIN recetas r ON dr.id_receta = r.id
    ...
    loop_ingredientes:LOOP
        FETCH cur_ingredientes INTO v_id_materia_prima, v_cantidad_necesaria;
        IF done THEN
            LEAVE loop_ingredientes;
        END IF;

        -- Descontar el stock de la materia prima considerando la cantidad del producto en la orden
        UPDATE materia_prima 
        SET existencia = existencia - (v_cantidad_necesaria * p_cantidad)
        WHERE id = v_id_materia_prima;
...
(7, 'KENDERSON', 'TORREALBA', NULL, 1, 'V-19572137');
...
--
-- Disparadores `entradas_materia_prima`
--
DELIMITER $$
CREATE TRIGGER `actualizarStock` AFTER INSERT ON `entradas_materia_prima` FOR EACH ROW BEGIN
    UPDATE `materia_prima`
...
(10, 70124700, 1, '2025-05-21 23:02:35', 0, 'delivery');
...
(46, NULL, 'Carne', '2c51307c-9d9f-41fb-9419-1e61a44891f0.jpeg', 0.56, NULL, 1, 'adicional'),
(47, NULL, 'Papitas', 'DIABLITOS-UNDERWOOD.jpg', 0.25, NULL, 1, 'adicional');
...
(9, 41, 10, '1', 'Sin cebolla', 'Tocineta', 1);
...
CREATE TABLE `usuario` (
  ...
  `token` varchar(45) NOT NULL DEFAULT 'token',
...
(11, 10, 'Alejandro', 'Alejandro202*', 'Vargas', 1, '1', 'garnicaluis391@gmail.com', '8848', '2025-05-25 22:18:49'),
